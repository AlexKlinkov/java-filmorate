...
public class ReviewDbStorage implements ReviewStorage {
    ...
    public Review update(Review review) {
        if (review.getReviewId() == null) {
            throw new NotFoundExceptionFilmorate("Передан пустой ID отзыва");
        } else if (getById(review.getReviewId()) == null) {
            throw new NotFoundExceptionFilmorate("Не существует отзыва на фильм с идентификатором № " + review.getReviewId());
        }
        jdbcTemplate.update(SQL_UPDATE_REVIEW,
                review.getContent(),
                review.getIsPositive(),
    ...
    public void deleteById(int reviewId) {
        if (reviewId == 0) {
            ...
            throw new NotFoundExceptionFilmorate("Не найден отзыв на фильм с идентификатором № " + reviewId);
        } else {
    ...
    public Review getById(int reviewId) {
        try {
            ...
            throw new NotFoundExceptionFilmorate("В Filmorate отсутствует отзыв на фильм с идентификатором № " + reviewId);
        }
    }
    ...
    private Review mapRowToReview(ResultSet resultSet, int i) throws SQLException {
        if (resultSet.getRow() == 0) {
            throw new NotFoundExceptionFilmorate("Отзыв на фильм не найден в storage.");
        }
