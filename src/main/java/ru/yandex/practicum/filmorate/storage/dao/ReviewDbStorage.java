...
public class ReviewDbStorage implements ReviewStorage {
    ...
    private static final String SQL_DELETE_LIKE_REVIEW = "DELETE FROM like_review WHERE review_id = ?";
    ...
    private static final String SQL_GET_REVIEW_WITHOUT_ID_LIMIT = "SELECT * FROM reviews ORDER BY (useful) " +
            "DESC LIMIT ?";
    ...
    public void deleteById(int reviewId) {
        if (reviewId == 0) {
            ...
            jdbcTemplate.update(SQL_DELETE_LIKE_REVIEW, reviewId);
    ...
    public List<Review> getReviewsForFilm(Long filmId, int count) {
        if (filmId == 0) {
            return jdbcTemplate.query(SQL_GET_REVIEW_WITHOUT_ID_LIMIT, this::mapRowToReview, count);
        }
        return jdbcTemplate.query(SQL_GET_REVIEWS_FOR_FILM, this::mapRowToReview, filmId, count);
    }
